...
import { createAgentHandler, listAgentsHandler, revokeAgentHandler } from "./handlers/agents";
import { authCallbackHandler, authStartHandler, dashboardHandler, logoutHandler, meHandler } from "./handlers/auth";
...
app.get("/auth/:provider/start", authStartHandler);
app.get("/auth/:provider/callback", authCallbackHandler);
app.post("/auth/logout", logoutHandler);
app.get("/v1/me", requireUser, meHandler);
app.get("/v1/bootstrap/status", bootstrapStatusHandler);
app.post("/v1/bootstrap/agent", bootstrapAgentHandler);
app.get("/v1/agents", requireUser, listAgentsHandler);
app.post("/v1/agents", requireUser, createAgentHandler);
app.delete("/v1/agents/:id", requireUser, revokeAgentHandler);
